...
function useGoogleAuth() {
  ...
  // טען את ספריית Google וברגע שהיא מוכנה — אתחל
  React.useEffect(() => {
  ...
  const loginWithGoogle = () => {
    if (!window.google?.accounts?.id) return;
    // הצג את חלון ההתחברות הרשמי של גוגל (One Tap / בחירת חשבון)
    window.google.accounts.id.prompt((notification) => {
      // אם One Tap נחסם/לא הוצג — נופלים חזרה לכפתור הרשמי הנסתר
      if (notification.isNotDisplayed?.() || notification.isSkippedMoment?.()) {
        const hiddenBtn = document.getElementById("google-signin-fallback-btn");
  ...
  return { user, skipped, loginWithGoogle, logout, skip, googleReady, handleCredentialResponse };
}

// כפתור אמיתי ומוסתר של גוגל — נדרש כי דפדפנים לפעמים חוסמים את ה-One Tap,
// אז אם זה קורה אנחנו "מקליקים" על הכפתור הרשמי בשבילכם
function GoogleFallbackButton({ googleReady }) {
  const ref = React.useRef(null);
  React.useEffect(() => {
    if (!googleReady || !ref.current || !window.google?.accounts?.id) return;
    window.google.accounts.id.renderButton(ref.current, { theme: "outline", size: "large", type: "standard" });
  }, [googleReady]);
  return <div id="google-signin-fallback-btn" ref={ref} style={{ position: "absolute", opacity: 0, pointerEvents: "none", top: -9999 }} />;
}
...
export default function Home() {
  ...
  if (!user && !skipped) {
    return (
      <>
        <GoogleFallbackButton googleReady={googleReady} />
        <LandingScreen onLogin={loginWithGoogle} onSkip={skip} />
      </>
    );
  }
                ...
                <div style={{
                  ...
                  {user && (
                    <>
                      ...
                      <div
                        onClick={() => { setSelectedCategory("היסטוריה"); setUserMenuOpen(false); }}
