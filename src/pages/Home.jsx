...
export default function Home() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!valid) {
          document.body.innerHTML = '<pre style="direction:ltr;font-family:monospace;padding:20px;">{"error":"Invalid or inactive API key"}</pre>';
          return;
        }
        ...
        document.body.innerHTML = `<pre style="direction:ltr;font-family:monospace;padding:20px;white-space:pre-wrap;word-break:break-all;">${JSON.stringify(moviesData)}</pre>`;
      } catch {
        document.body.innerHTML = '<pre style="direction:ltr;font-family:monospace;padding:20px;">{"error":"Server error"}</pre>';
      }
    })();
  }, [slug]);
  ...
  useEffect(() => {
    ...
    if (slug) {
      ...
      if (movies.some(m => m.series_name === decoded)) {
        ...
        // אם יש גם episode ב-URL — פתח נגן ישירות
        if (episode) {
          const epDecoded = decodeURIComponent(episode).replace(/-/g, " ");
          const epNum = parseInt(epDecoded.replace(/[^0-9]/g, "")) || 1;
      ...
      // בדוק אם זה סרט לפי shortId
      const parts = slug.split("-");
      const shortId = parts[parts.length - 1];
      const found = movies.find(m => m.id.startsWith(shortId));
      if (found) {
        setSelectedMovie(found);
        // אם יש episode — פתח נגן ישירות (לסרט עם URL ישיר)
  ...
  const openWithKalturaRefresh = async (movie) => {
    ...
    if (movie.series_name && movie.episode_number) {
      const serSlug = encodeURIComponent(movie.series_name.replace(/ /g, "-"));
      const epSlug = encodeURIComponent(`פרק-${movie.episode_number}`);
      window.history.replaceState(null, "", `/zovex/${serSlug}/${epSlug}`);
    } else if (movie.title && !movie.series_name) {
      const movieSlug = encodeURIComponent((movie.title || "").replace(/ /g, "-")) + "-" + movie.id.slice(0, 6);
      window.history.replaceState(null, "", `/zovex/${movieSlug}/watch`);
    }
