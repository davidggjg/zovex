...
export default function Home() {
  ...
  // live stream — נשמר ב-movies.json כאובייקט עם id: "live_stream"
  const [liveUrl, setLiveUrl] = useState("");
  const [liveUrlInput, setLiveUrlInput] = useState("");
  const [showLivePlayer, setShowLivePlayer] = useState(false);
  const [liveSaving, setLiveSaving] = useState(false);
  const liveActive = !!liveUrl;
  ...
  const loadMovies = async () => {
    ...
    try {
      ...
      const liveEntry = all.find(m => m.id === "live_stream");
      setLiveUrl(liveEntry?.video_url || "");
      setLiveUrlInput(liveEntry?.video_url || "");
      setMovies(all.filter(m => m.id !== "live_stream"));
    } catch {}
    setLoading(false);
  };
  ...
  useEffect(() => {
    loadLiveFromGitHub();
    // בדוק כל 30 שניות אם יש שידור חי
          ...
          <div>
            <div style={cardStyle}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {liveActive ? "שידור חי פעיל" : "אין שידור חי פעיל"}
              </div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                <button onClick={async () => {
                  ...
                  try {
                    const all = await Movie.list("-created_date", 2000);
                    const filtered = all.filter(m => m.id !== "live_stream");
                    const liveEntry = { id: "live_stream", video_url: liveUrlInput.trim(), title: "שידור חי", created_date: new Date().toISOString() };
                    await Movie.saveAll([liveEntry, ...filtered]);
                    setLiveUrl(liveUrlInput.trim());
                    setFormStatus({ type: "success", message: "✅ שידור חי הופעל! (יופיע לכולם תוך שניות)" });
                    setTimeout(() => loadLiveFromGitHub(), 3000);
                  ...
                  {liveSaving ? "⏳ שומר..." : "🔴 הפעל שידור חי"}
                </button>
                <button onClick={async () => {
                  setLiveSaving(true);
                  try {
                    const all = await Movie.list("-created_date", 2000);
                    await Movie.saveAll(all.filter(m => m.id !== "live_stream"));
                    setLiveUrl("");
                    setLiveUrlInput("");
                  ...
                  setLiveSaving(false);
                  setTimeout(() => setFormStatus({ type: "", message: "" }), 2000);
                }} disabled={liveSaving} style={{ flex: 1, background: "#f5f5f7", color: "#333", border: "1.5px solid #d2d2d7", borderRadius: 12, padding: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", opacity: liveSaving ? .6 : 1 }}>
                  ⏹ עצור שידור
                </button>
              </div>

              {formStatus.message && <div style={{ borderRadius: 10, padding: "10px 12px", fontSize: 12, background: "#f0fff4", color: "#1a7a3a", marginBottom: 12 }}>{formStatus.message}</div>}
  ...
  return (
    <>
      ...
      {/* ── Live Banner ── */}
      {liveActive && !showLivePlayer && (
        <div style={{ background: "linear-gradient(135deg,#e50914,#c0000f)", color: "#fff", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", direction: "rtl", fontFamily: "Arial,sans-serif", position: "sticky", top: 0, zIndex: 999 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ width: 10, height: 10, borderRadius: "50%", background: "#fff", display: "inline-block", animation: "livePulse 1.5s ease-in-out infinite" }} />
            <span style={{ fontWeight: 900, fontSize: 14 }}>🔴 שידור חי</span>
            <span style={{ fontSize: 13, opacity: .9 }}>— יש שידור חי עכשיו הצטרפו!</span>
          </div>
          <button onClick={() => setShowLivePlayer(true)} style={{ background: "#fff", color: "#e50914", border: "none", borderRadius: 20, padding: "6px 16px", fontSize: 13, fontWeight: 900, cursor: "pointer", flexShrink: 0 }}>צפה עכשיו ▶</button>
        </div>
      )}
      ...
      {showLivePlayer && (
        <div style={{ position: "fixed", inset: 0, background: "#000", zIndex: 9999, display: "flex", flexDirection: "column" }}>
          <style>{SPIN}</style>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: "rgba(0,0,0,.85)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#fff" }}>
              <span style={{ width: 10, height: 10, borderRadius: "50%", background: "#e50914", display: "inline-block", animation: "livePulseDot 1.5s ease-in-out infinite" }} />
              <span style={{ fontWeight: 900, fontSize: 15 }}>שידור חי</span>
            </div>
            <button onClick={() => setShowLivePlayer(false)} style={{ background: "none", border: "none", color: "#fff", fontSize: 28, cursor: "pointer", lineHeight: 1, padding: "0 4px" }}>×</button>
          </div>
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", width: "100%" }}>
            <iframe
              srcDoc={`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{margin:0;padding:0;box-sizing:border-box;background:#000}html,body{width:100%;height:100%;overflow:hidden}video{width:100vw;height:100vh;object-fit:contain;display:block}</style></head><body><video id="v" controls autoplay playsinline></video><script src="https://cdnjs.cloudflare.com/ajax/libs/hls.js/1.4.12/hls.min.js"><\/script><script>var v=document.getElementById('v');var src="${liveUrl}";function tryPlay(){v.play&&v.play().catch(function(){});}if(window.Hls&&Hls.isSupported()){var hls=new Hls({enableWorker:false,lowLatencyMode:true});hls.loadSource(src);hls.attachMedia(v);hls.on(Hls.Events.MANIFEST_PARSED,tryPlay);}else if(v.canPlayType('application/vnd.apple.mpegurl')){v.src=src;v.addEventListener('loadedmetadata',tryPlay);}<\/script></body></html>`}
          ...
          {filteredItems.length > visibleCount && (
            <div style={{ textAlign: "center", padding: "20px 0 40px" }}>
              <button
                ...
                style={{ background: "#e50914", color: "#fff", border: "none", borderRadius: 50, padding: "12px 32px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}
              >
                טען עוד ({filteredItems.length - visibleCount} נותרו)
              </button>
            </div>
          )}
        </main>
