...
export default function Home() {
  ...
  // live stream
  const [liveUrl, setLiveUrl] = useState(() => ls("zovex_live_url", ""));
  const [liveUrlInput, setLiveUrlInput] = useState(() => ls("zovex_live_url", ""));
          ...
          <div>
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                <button onClick={() => {
                  lsSet("zovex_live_url", liveUrlInput.trim());
                  setLiveUrl(liveUrlInput.trim());
                  setFormStatus({ type: "success", message: "✅ שידור חי הופעל!" });
                  setTimeout(() => setFormStatus({ type: "", message: "" }), 2000);
                }} style={{ flex: 1, background: "#e50914", color: "#fff", border: "none", borderRadius: 12, padding: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  🔴 הפעל שידור חי
                </button>
                <button onClick={() => {
                  lsDel("zovex_live_url");
                  setLiveUrl("");
                  setLiveUrlInput("");
                  setFormStatus({ type: "success", message: "⏹ שידור הופסק" });
                  setTimeout(() => setFormStatus({ type: "", message: "" }), 2000);
                }} style={{ flex: 1, background: "#f5f5f7", color: "#333", border: "1.5px solid #d2d2d7", borderRadius: 12, padding: 12, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                  ⏹ עצור שידור
                </button>
              </div>
