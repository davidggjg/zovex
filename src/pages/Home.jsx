...
function useGoogleAuth() {
  ...
  const loginWithGoogle = () => {
    const clientId = "GOOGLE_CLIENT_ID_HERE";
    const redirect = encodeURIComponent(window.location.origin + "/zovex/");
  ...
  // בדוק אם חזרנו מ-Google OAuth עם access_token ב-URL hash
  React.useEffect(() => {
    ...
    fetch(`https://www.googleapis.com/oauth2/v3/userinfo?access_token=${token}`)
      .then(r => r.json())
      .then(info => {
        const userData = { id: info.sub, name: info.name, email: info.email, picture: info.picture };
        try { localStorage.setItem("zovex_user", JSON.stringify(userData)); } catch {}
        setUser(userData);
        window.history.replaceState(null, "", "/zovex/");
      }).catch(() => {});
  }, []);

  return { user, skipped, loginWithGoogle, logout, skip };
}
...
export default function Home() {
  ...
  return <HomeMain user={user} onLogout={logout} />;
}

function HomeMain({ user, onLogout }) {
  ...
  const saveHistory = React.useCallback(async (mediaId, title, thumbnailUrl) => {
    if (!user) return;
    await apiCall("/api/history", "POST", { media_id: mediaId, title, thumbnail_url: thumbnailUrl || "" }, user.id);
  }, [user]);
  ...
  const allCategories = useMemo(() => {
    ...
    const base = ["הכל", ...new Set([...categories, ...from])];
  ...
  return (
    <>
      ...
      {/* ── Live Player fullscreen — נפתח מתוך קטגוריית "שידורים חיים" ── */}
      {showLivePlayer && (
        <div style={{ position: "fixed", inset: 0, background: "#000", zIndex: 9999, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: "rgba(0,0,0,.85)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#fff" }}>
              ...
              <span style={{ fontWeight: 900, fontSize: 15 }}>{showLivePlayer.title}</span>
            </div>
            <button onClick={() => setShowLivePlayer(null)} style={{ background: "none", border: "none", color: "#fff", fontSize: 28, cursor: "pointer", lineHeight: 1, padding: "0 4px" }}>×</button>
          </div>
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", width: "100%" }}>
            <iframe
              srcDoc={`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{margin:0;padding:0;box-sizing:border-box;background:#000}html,body{width:100%;height:100%;overflow:hidden}video{width:100vw;height:100vh;object-fit:contain;display:block}</style></head><body><video id="v" controls autoplay playsinline></video><script src="https://cdnjs.cloudflare.com/ajax/libs/hls.js/1.4.12/hls.min.js"><\/script><script>var v=document.getElementById('v');var src="${showLivePlayer.video_url}";function tryPlay(){v.play&&v.play().catch(function(){});}if(window.Hls&&Hls.isSupported()){var hls=new Hls({enableWorker:false,lowLatencyMode:true});hls.loadSource(src);hls.attachMedia(v);hls.on(Hls.Events.MANIFEST_PARSED,tryPlay);}else if(v.canPlayType('application/vnd.apple.mpegurl')){v.src=src;v.addEventListener('loadedmetadata',tryPlay);}<\/script></body></html>`}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            ...
            {user ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                <img src={user.picture} alt={user.name} style={{ width: 34, height: 34, borderRadius: "50%", border: "2px solid #e50914", cursor: "pointer", flexShrink: 0 }} onClick={() => setSelectedCategory("היסטוריה")} title={`${user.name} — לחץ לצפייה בהיסטוריה`} onError={e => e.target.style.display = "none"} />
                <button onClick={onLogout} style={{ background: "none", border: "1px solid #ddd", borderRadius: 20, padding: "5px 12px", fontSize: 12, color: "#888", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}>יציאה</button>
              </div>
            ) : (
              <div style={{ flexShrink: 0, fontSize: 11, color: "#aaa" }}>אורח</div>
            )}
          </div>
...
function NetflixRows({ movies, seriesMap, liveChannels, allCategories, selectedCategory, searchTerm, isDesktop, handleItemClick, history, user }) {
  const q = searchTerm.toLowerCase();
  ...
  } else if (selectedCategory !== "שידורים חיים") {
  ...
  // אם בוחרים "היסטוריה" — הצג רק את ההיסטוריה
  if (selectedCategory === "היסטוריה") {
    return (
      <div style={{ paddingTop: 8 }}>
        {continueWatchingItems.length > 0 ? (
          <NetflixRow title="📋 היסטוריית צפייה" items={continueWatchingItems} isDesktop={isDesktop} handleItemClick={handleItemClick} isLiveRow={false} />
        ) : (
  ...
  return (
    <div style={{ paddingTop: 8 }}>
      {/* שורת המשך צפייה — ראשונה תמיד אם יש */}
      {user && continueWatchingItems.length > 0 && (selectedCategory === "הכל" || selectedCategory === "שידורים חיים") && (
        <NetflixRow title="▶️ המשך צפייה" items={continueWatchingItems} isDesktop={isDesktop} handleItemClick={handleItemClick} isLiveRow={false} />
      )}
      {rowsToShow.map(row => (
        <NetflixRow
          key={row.title}
