...
export default function Home() {
  ...
  const allCategories = useMemo(() => {
    ...
    // קטגוריית שידורים חיים מוצגת בנפרד עם אייקון עין, רק אם יש שידור פעיל
    return liveActive ? [...base, "שידורים חיים"] : base;
  }, [movies, categories, liveActive]);
                ...
                {filteredItems.slice(0, visibleCount).map((item) => {
                  ...
                  return (
                    <div key={isSer ? "s-" + item.name : item.id} onClick={() => handleItemClick(item, isSer)} style={{ cursor: "pointer", display: "flex", flexDirection: "column", gap: 7 }}>
                      <div style={{ borderRadius: 12, overflow: "hidden", height: isDesktop ? "360px" : "260px", background: isLive ? "#1a1a1a" : "#e8e8e8", position: "relative", flexShrink: 0, border: isLive ? "2px solid #e50914" : "none" }}>
                        {isLive ? (
                          <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, background: "linear-gradient(135deg,#1a1a1a,#2a0a0c)" }}>
                            <Eye size={36} color="#e50914" strokeWidth={2} />
                            <span style={{ fontSize: 11, color: "#fff", fontWeight: 700, textAlign: "center", padding: "0 10px" }}>צופה בשידורים החיים שיש עכשיו</span>
                          </div>
                        ) : item.thumbnail_url
                        ...
                        {isSer && !isLive && <div style={{ position: "absolute", top: 8, right: 8, background: "rgba(0,0,0,.65)", borderRadius: 8, padding: "3px 8px", fontSize: 10, color: "#fff", fontWeight: 700 }}>סדרה</div>}
                        {isLive && (
                          <div style={{ position: "absolute", top: 8, right: 8, background: "#e50914", borderRadius: 8, padding: "3px 8px", fontSize: 10, color: "#fff", fontWeight: 900, display: "flex", alignItems: "center", gap: 4 }}>
                            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#fff", display: "inline-block", animation: "livePulseDot 1.5s ease-in-out infinite" }} />
                            LIVE
                          </div>
                        )}
                      </div>
