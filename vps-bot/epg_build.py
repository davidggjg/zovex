...
# יוצג לו לוח.
#
# נבדקו ולא נמצאו בשום מקור מהארבעה (אין להמציא להם קוד):
#   hot-hbo        — אין ערוץ HBO ברשימות של וואלה.
#   cellcom-kids   — אין ערוץ ילדים של סלקום ברשימות של וואלה.
#   Hfc            — ה-slug של ערוץ ה-UFC אצלנו; אין ערוץ UFC/לחימה שם.
MAP = {
    # חדשות וערוצים
    ...
    # להם לוח (404 על /epg/5plus.json ועל /epg/5max.json). וואלה קוראת
    # לראשון "ספורט 5+" (קוד 204, מופיע בשני הספקים עם לוח זהה תו-בתו)
    # ולשני "5MAX" (קוד 4662, ספק 2 בלבד).
    # שניהם אומתו כערוצים נפרדים ולא כשמות נוספים לערוץ שכבר ממופה: הלוח
    # של 204 שונה לגמרי מ-3600 (5LIVE), מ-66/321 (GOLD), מ-521 ומ-289,
    # ו-4662 משדר MLB וטניס בשעות שבהן האחרים משדרים יורוליג ופורמולה.
    "5plus": ("w", 204), "5max": ("w", 4662),
