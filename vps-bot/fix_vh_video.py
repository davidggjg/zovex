...
מה זה עולה:
    המרה בזמן אמת. סמולוויל הוא SD, ו-veryfast מקודד אותו בכמה מונים
    מהזמן האמיתי על שני ליבות, כלומר מקטע של 10 שניות נבנה בשנייה-שתיים.
    HEVC ייכנס לאותו מסלול ויהיה כבד יותר. מספר הליבות למקטע נשלט
    ב-VODFIX_VIDEO_THREADS (ברירת מחדל 2) כדי שצופים במקביל לא יחנקו
    את השרת.
