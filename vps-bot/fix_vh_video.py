...
מה זה עולה — נמדד בייצור אחרי ההתקנה, ולא הוערך:
    ...
    אבל **המעבד הוא לא הצוואר**, וזה נמדד בנפרד. אותו מקטע (15.4 שניות,
    608x336, mpeg4 → H.264) על שני חוטים:

        veryfast  crf23   0.87s   פי 17.7 מהזמן האמיתי
        superfast crf23   0.72s   פי 21.4
        ultrafast crf23   0.60s   פי 25.6   (וקובץ גדול ב-50%)

    ובמקביל, משיכת 2MB מהמקור בטלגרם דרך /stream:

        היסט 5MB     3.01s   (2.0s עד הבייט הראשון)
        היסט 60MB    1.89s   (0.9s עד הבייט הראשון)
        היסט 200MB   1.90s   (0.9s עד הבייט הראשון)

    כלומר מתוך 4–8 השניות, הקידוד הוא כשנייה אחת וכל השאר הוא המתנה
    לטלגרם. לכן אין טעם לגעת ב-VODFIX_X264_PRESET — ultrafast יחסוך
    0.27 שניות מתוך 4–8, ויעלה 50% בגודל. הברגים קיימים, אבל הם לא
    המקום להסתכל בו.

    מה שכן עוזר הוא לא לשלם את ההמתנה הזאת פעמיים: fix_vh_cache שומר
    את המקטע המומר, וכך גם הקידוד וגם המשיכה מטלגרם נחסכים בכל בקשה
    חוזרת. מאותה סיבה גם קידוד מראש של כל הספרייה הוא רעיון רע: הוא
    חסום על הורדה מטלגרם, לא על מעבד, וייקח שעות על שעות בלי שום יתרון
    על פני המטמון.
