...
_MKV_SEG_TARGET = float(os.environ.get("MKV_SEG", "10"))
...
def _vf_seg_args(chat_id: int, message_id: int, info: dict, seg: int) -> list:
    ...
    return [
        ...
        "-ss", f"{seek_at:.3f}", "-to", f"{start + dur:.3f}", "-i", src,
...
async def vodfix_playlist(chat_id: int, message_id: int, request: Request,
    ...
    check_hotlink(request)
    _vf_check_sig(chat_id, message_id, exp, sig)
    info = await _vf_info_or_mkv(chat_id, message_id)
...
async def vodfix_segment(chat_id: int, message_id: int, seg: int,
    ...
    start, dur = segs[seg][0], segs[seg][1]
...
async def vodfix_info(chat_id: int, message_id: int, request: Request,
    ...
    try:
        ...
        # ‎/vh ולא ‎/vt: אותה המרה של קול, אבל פלייליסט VOD שלם, ולכן
