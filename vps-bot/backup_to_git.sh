...
#
# הרשימה הורחבה בסקירת אבטחה. הסקריפט דוחף למאגר **ציבורי** לא רק את
# main.py ו-admin.html אלא כל קובץ ‎/root/*.py שעוד לא שם — ומי שקורא את
# המאגר קורא גם אותו. ארבע התבניות המקוריות לא תפסו אף אחד מהמפתחות
# שבאמת בשימוש כאן: Gemini, Google/YouTube, ה-api_hash של טלגרם ומחרוזות
# session. כל אחד מהם שהיה נכתב בטעות לסקריפט ב-/root היה עולה לרשת.
SECRET_PATTERNS=(
  '[0-9]{8,10}:AA[A-Za-z0-9_-]{30,}'   # טוקן בוט טלגרם
  'ghp_[A-Za-z0-9]{30,}'               # טוקן GitHub
  'github_pat_[A-Za-z0-9_]{30,}'
  '-----BEGIN [A-Z ]*PRIVATE KEY-----'
  'AIza[0-9A-Za-z_-]{35}'              # מפתח Google / YouTube
  'AQ\.Ab8R[0-9A-Za-z_-]{20,}'         # מפתח Gemini
  'api_hash[^=\n]{0,8}=[^\n]{0,4}[0-9a-f]{32}'   # api_hash של טלגרם כשהוא כתוב בקוד
  '1[A-Za-z0-9_-]{300,}={0,2}'         # מחרוזת session של טלגרם (pyrogram)
  'sk-(ant-)?[A-Za-z0-9_-]{30,}'       # מפתחות API בסגנון sk-
  '(PANEL|EDITOR)_PASSWORD[^=\n]{0,4}=[^\n]{0,4}[A-Za-z0-9]{8,}'  # סיסמת פאנל כתובה בקוד
)
...
scan_secrets() {
  ...
  for f in "$@"; do
    for pat in "${SECRET_PATTERNS[@]}"; do
      # ‎-e חובה: תבנית שמתחילה ב-"-" (המפתח הפרטי) נקראת בלעדיו כאופציה
      # של grep, שנכשל, והשגיאה מוסתרת — כך הבדיקה הזאת לא תפסה מעולם.
      if grep -Eq -e "$pat" -- "$f" 2>/dev/null; then
