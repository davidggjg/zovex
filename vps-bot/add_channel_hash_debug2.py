...
MARKER = "DEBUG_CHANNEL_HASH2_ENDPOINT"

ADDITION = '''

# ── ''' + MARKER + ''' — זמני, קריאה-בלבד. להסיר אחרי שימוש. ──────────────
@api.get("/debug/channel-hash2")
async def _debug_channel_hash2():
    try:
        if not _stream_bots:
            return {"error": "_stream_bots ריקה"}
        b = _stream_bots[0]
...
def undo():
    baks = sorted(pathlib.Path("/opt/zovex-bot").glob("main.py.bak-chdebug2-*"))
...
def main():
    ...
    os.unlink(tmp)
    bak = f"/opt/zovex-bot/main.py.bak-chdebug2-{datetime.datetime.now():%Y%m%d-%H%M%S}"
    shutil.copy2(TARGET, bak)
    TARGET.write_text(out, encoding="utf-8")
    print("✅ הוחל.")
    print(f"   גיבוי: {os.path.basename(bak)}")
    print("   עכשיו:   systemctl restart zovex-bot")
    print("   ואז:     curl -s http://127.0.0.1:8000/debug/channel-hash2")
    print("   נסיגה:   python3 add_channel_hash_debug2.py --undo")
