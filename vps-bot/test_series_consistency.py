...
# ── השלמת מזהה ───────────────────────────────────────────────────────────────
ch, clash, tie, defer, st = F.plan([ep(1, "א", tid=100), ep(2, "א"), ep(3, "א")])
...
ch, clash, tie, defer, st = F.plan([ep(1, "א", tid=100), ep(2, "א", tid=200),
                             ep(3, "א")])
...
# ── איחוד קטגוריה ────────────────────────────────────────────────────────────
ch, clash, tie, defer, st = F.plan([ep(1, "ב", cat="אנימה"), ep(2, "ב", cat="אנימה"),
                             ep(3, "ב", cat="סדרות")])
eq(fields(ch), {(3, "category"): "אנימה"}, "הרוב קובע")
eq(tie, [], "אין תיקו")

ch, clash, tie, defer, st = F.plan([ep(1, "ב", cat="אנימה"), ep(2, "ב", cat="סדרות")])
...
ch, clash, tie, defer, st = F.plan([ep(1, "ב", cat=""), ep(2, "ב", cat="")])
eq(ch, [], "סדרה שכולה בלי קטגוריה — אין ממה להסיק")
eq(tie, [], "ולא תיקו")

# וסדרה עקבית לגמרי אינה נספרת כ"אוחדה"
ch, _, _, _, st = F.plan([ep(1, "ב", cat="אנימה"), ep(2, "ב", cat="אנימה")])
eq(st.get("סדרות שהקטגוריה אוחדה בהן"), None,
   "סדרה שלא השתנתה לא נספרת")
ch, _, _, _, st = F.plan([ep(1, "ב", cat="אנימה"), ep(2, "ב", cat="")])
eq(st.get("סדרות שהקטגוריה אוחדה בהן"), 1, "וסדרה שכן — נספרת")

# ── הורדה-לגנרי ברוב: לא מאחדים, דוחים ל-fix_categories ─────────────────────
# Asfur נמדדה עם 21 פרקים ב"סדרות ישראליות" ורוב ב"סדרות". "סדרות"
# גנרי, "סדרות ישראליות" ספציפי — והרוב לא אמור לגרור את הספציפי למטה.
ch, clash, tie, defer, st = F.plan(
    [ep(i, "עספור", cat="סדרות ישראליות") for i in range(21)]
    + [ep(100+i, "עספור", cat="סדרות") for i in range(30)])
eq(ch, [], "רוב גנרי מול מיעוט ספציפי — לא מאחדים")
eq(len(defer), 1, "אלא דוחים ל-fix_categories")
eq(defer[0][0], "עספור", "בשם הסדרה")

# אבל רוב **ספציפי** מול מיעוט גנרי כן מאחד — זה הכיוון הנכון
ch, _, _, defer, _ = F.plan(
    [ep(i, "עספור", cat="סדרות ישראליות") for i in range(30)]
    + [ep(100+i, "עספור", cat="סדרות") for i in range(21)])
eq({(c[0]["id"], c[4]) for c in ch},
   {(100+i, "סדרות ישראליות") for i in range(21)},
   "רוב ספציפי מושך את הגנרי אליו")
eq(defer, [], "וזה לא נדחה")

# ושתי קטגוריות גנריות בלבד — כן מאחדים, אין ספציפי להגן עליו
ch, _, _, defer, _ = F.plan(
    [ep(i, "עספור", cat="סרטים") for i in range(3)]
    + [ep(100+i, "עספור", cat="סדרות") for i in range(2)])
eq({c[4] for c in ch}, {"סרטים"}, "גנרי מול גנרי — הרוב מנצח")
eq(defer, [], "ולא נדחה")
...
eq(by[3].get("category"), "אנימה",
   "הפרק הספציפי לא נגרר לגנרי — נדחה ל-fix_categories")
