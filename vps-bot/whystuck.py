...
PATTERNS = [
    ...
    # "ויתר" היא השורה שבה המחולל חוזר והגוף נמסר קטוע — כלומר תקיעה אחת
    # שצופה ראה. חיפוש "חרג מתקציב הקיר" בלבד פספס אותה לגמרי: זו רק אחת
    # משתי החריגות שהקוד עשוי לזרוק, ובפועל נמדדו 20 ויתורים מול 0 קירות.
    (r"ויתר אחרי", "🧱 חלון ויתר — הגוף נמסר קטוע = תקיעה אחת אצל הצופה"),
    (r"חרג מתקציב הקיר", "🧱 חלון הגיע לקיר של 30 שניות"),
    (r"timeouts רצופים",
