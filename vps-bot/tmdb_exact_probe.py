...
ואת גודל הבקשה הכלי הזה מודד בעצמו. האמדן הראשון שלי היה 1,850
טוקנים לבקשה, והוא היה גבוה מדי: הוא הניח 8 מועמדים כל אחד עם תקציר
מלא, ובפועל TMDB מחזיר ~2 מועמדים ליחידה. הנמדד על 100 יחידות:

    כמו עכשיו        912 טוקנים ליחידה
    overview ל-80    501
    בלי overview     370

935 × 912 ≈ 850,000 טוקנים — פי 1.7 מהמכסה היומית. ואחרי שריצת
האימות (234 יחידות) בלעה את רוב היום, הסריקה המלאה התחילה על דלי
ריק. זה בדיוק ההיתקעות שנמדדה: 68 שעות בקצב שנצפה.
...
לכן שתי שאלות, ושתיהן נמדדות כאן **בלי טוקן אחד של מודל**:
  ...
  2. כמה מועמדים נחסכים מסינון לפי סוג: יחידה שהיא סדרה אצלנו לא
     ...
     (על סט האימות זה חסך 0% — הוא כמעט כולו סרטים, ולסרט לא מסננים
     כלום. הערך שלו הוא על הסדרות, שהן רוב מה שחסר.)

והנמדד על 100 יחידות מסט האימות: 74 נפתרו בלי מודל, ומתוכן 74 נכונות
מול המזהה שכבר היה בקטלוג. כלומר 74% מהעבודה בחינם ובדיוק מלא, ורק
26% נשארות למודל.
